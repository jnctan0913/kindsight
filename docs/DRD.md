...
**Light surfaces everywhere (revised 2026-07-04, product owner decision, supersedes the night ritual rule).** All player screens including the ritual stay light: off-white `#F5FAFB` background, navy text, template-native. The aurora carries the ritual feeling through accents instead of a dark surface: the gradient progress ring, silhouette shimmer, and glow shadows. The only dark surface in the product is the big screen's reveal interstitial, where dimming the projector output is functional (it darkens the physical room).

**Ritual flow revision (2026-07-04, product owner decision, supersedes 3.2 states D and E).** Notes are not revealed one by one. Completing the hold opens the full wall directly: all notes cascade in with a staggered entrance (140ms delay per card, skipped under reduced motion), and each card carries one quiet share affordance: a 44px round icon button (screen icon) in its top-right corner. Tapping it toggles that note onto the big screen; when shared, the button fills teal with a check inside the icon and the card gains the aurora glow. A single hint line under the wall header explains the gesture once, so no prompt text repeats per card, and the button exposes state to assistive tech via `aria-pressed`. Choices stay editable from this screen until the room ends (resolved Q2), and the default remains not shared (PRD resolved Q7). The hold-to-unlock gains meaning: each of the three breaths surfaces one priming line ("Someone in this room noticed you." / "These words were written only for you." / "Take them in slowly."), replacing the generic breathe-in/breathe-out coaching. These lines are the emotional setup for the wall that follows.

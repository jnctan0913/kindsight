...
**Ritual flow revision (2026-07-04, product owner decision, supersedes 3.2 states D and E).** Notes are not revealed one by one. Completing the hold opens the full wall directly: all notes cascade in with a staggered entrance (140ms delay per card, skipped under reduced motion), and each card carries its own inline share toggle (No / Share) so opting in happens on the same screen. The hold-to-unlock gains meaning: each of the three breaths surfaces one priming line ("Someone in this room noticed you." / "These words were written only for you." / "Take them in slowly."), replacing the generic breathe-in/breathe-out coaching. These lines are the emotional setup for the wall that follows.
